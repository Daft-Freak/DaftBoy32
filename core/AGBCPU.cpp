...
uint32_t AGBCPU::readMem16(uint32_t addr) const
{
    ...
    auto aligned = addr & ~1;
    uint16_t val = readMem8(aligned) | (readMem8(aligned + 1) << 8);
...
{
    assert((addr & 1) == 0);
    return readMem8(addr) | (readMem8(addr + 1) << 8);
}

uint32_t AGBCPU::readMem32(uint32_t addr) const
{
    auto aligned = addr & ~3;
    uint32_t val = readMem8(aligned) | (readMem8(aligned + 1) << 8) | (readMem8(aligned + 2) << 16) | (readMem8(aligned + 3) << 24);
...
{
    assert((addr & 3) == 0);
    return readMem8(addr) | (readMem8(addr + 1) << 8) | (readMem8(addr + 2) << 16) | (readMem8(addr + 3) << 24);
}
